...
use crate::plugins::{prefab, serializable::Serializable};
...
pub struct PrefabCollection {
    pub items: HashMap<String, Handle<Scene>>,
    pub loaded: usize,
}
...
fn is_not_loaded(prefabs: Res<PrefabCollection>) -> bool {
    !prefabs.is_loaded()
}
...
impl Plugin for PrefabPlugin {
    fn build(&self, app: &mut bevy::app::App) {
        app.register_type::<Prefab>()
            ...
            .add_systems(First, resolve.run_if(is_not_loaded));
    }
}
...
fn resolve(
    ...
    for item in prefabs.items.values() {
        if scenes.contains(item) {
            loaded += 1;
            continue;
        }

        if prefabs.loaded != loaded {
            prefabs.loaded = loaded;
        }
        return;
    }
    ...
    commands.trigger(PrefabsLoadedEvent);
}
