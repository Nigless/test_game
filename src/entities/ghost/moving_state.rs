...
impl Plugin for MovingStatePlugin {
    fn build(&self, app: &mut App) {
        app.register_type::<MovingState>().add_systems(
            PreUpdate,
            (
...
fn enter(
    mut entity_q: Query<(&mut Collider, &mut Stats, &mut AnimationSequencer), Added<MovingState>>,
) {
...
fn exit(mut removed: RemovedComponents<MovingState>, mut entity_q: Query<&mut AnimationSequencer>) {
...
fn update(
    mut entity_q: Query<(&mut AnimationSequencer, &Velocity, &CharacterBody), With<MovingState>>,
) {
